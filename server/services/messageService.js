...
export async function sendTemplate(memberId, type, extraMergeData = {}) {
  const member = await Member.findById(memberId);
  if (!member) throw Object.assign(new Error("Member not found"), { statusCode: 404 });
  if (!member.phone) throw Object.assign(new Error("Member has no phone number"), { statusCode: 400 });

  const template = businessConfig.smsTemplates[type];
  if (!template) throw Object.assign(new Error(`Unknown SMS type: ${type}`), { statusCode: 400 });

  const mergeData = { firstName: member.name.split(" ")[0], ...extraMergeData };
  const body = template.replace(/\{(\w+)\}/g, (_, key) => mergeData[key] ?? `{${key}}`);
