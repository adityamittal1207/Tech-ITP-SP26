// Mock data for Tidewater Yoga + Surf — internally consistent fixtures.
// Seeded pseudo-random so charts don't reshuffle between renders.
...
const CHANNELS = ["Instagram", "Groupon", "Referral", "Walk-in"] as const;
...
function makeClient(i: number): Client {
  ...
  const tags = Array.from(new Set(range(tagsCount).map(() => pick([...ALL_TAGS]))));
...
export const CLIENTS: Client[] = range(280).map(makeClient);

export const COUNTS = STATUSES.reduce((acc, s) => {
...
// ---- Trend series ----
export const VISITS_TREND = range(30).map((i) => ({
  d: `D${i + 1}`,
  visits: Math.round(80 + Math.sin(i / 3) * 18 + rand() * 20),
  noShows: Math.round(2 + rand() * 6),
}));

// ---- Schedule heatmap: 7 days × 12 slots ----
export const DAYS = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];
export const SLOTS = ["6a", "7a", "8a", "9a", "10a", "12p", "4p", "5p", "6p", "7p", "8p"];
export const SCHEDULE_HEATMAP = DAYS.map((d) => ({
  day: d,
  cells: SLOTS.map((s) => {
...
export const COHORTS = MONTHS.map((m, idx) => ({
  month: m,
...
// ---- Per-class trend ----
export const CLASS_TREND = range(12).map((i) => ({
  wk: `W${i + 1}`,
...
// ---- RevPASH ----
export const REVPASH_TREND = range(12).map((i) => ({
  wk: `W${i + 1}`,
  revpash: +(11 + Math.sin(i / 2.5) * 1.8 + rand() * 1.2).toFixed(2),
}));

// ---- LTV by channel ----
export const CHANNEL_LTV = [
...
export const SEND_QUEUE = CLIENTS.filter((c) => c.status === "At-Risk" || c.status === "Win-back")
  .slice(0, 8)
  .map((c, i) => ({
    id: `q${i}`,
    client: c,
